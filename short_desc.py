...
class ShortDescription:
	...
	def describeGeneric(self, q, claims, opt):
		...
		if len(h) == 0:
			...
			if opt["fallback"] == 'manual_desc' and "descriptions" in self.main_data and opt["lang"] in self.main_data[
				"descriptions"]:
	...
	def labelItems(self, items, opt=None):
		...
		for q in i:
			...
			if opt["links"] == 'wikidata':
				...
				cb[p].append("<a href='/reasonator/?lang=" + opt[
					"reasonator_lang"] + "&q=" + q + "'" + linktarget + ">" + label + "</a>")
			elif opt["links"] == 'wiki':
				...
				if "local_links" in opt:
					...
					cb[p].append("<a href='#" + use_lang + "." + opt[
						"links"] + ".org/wiki/" + page + "'" + linktarget + ">" + label + "</a>")
			else:
				cb[p].append(label)
		return cb
